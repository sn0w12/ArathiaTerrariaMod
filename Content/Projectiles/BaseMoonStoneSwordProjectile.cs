...
using Arathia.Content.Items.Weapons;
...
namespace Arathia.Content.Projectiles
{
	...
	public class BaseMoonStoneSwordProjectile : ModProjectile
	{
        private MoonStoneSword parentSword;
		...
		public int dustId = ModContent.DustType<TsukiDust>();
		public int projectileType = ModContent.ProjectileType<MoonStoneProjectile>();
		...
		private float prepTime => time / Owner.GetTotalAttackSpeed(Projectile.DamageType);
		private float execTime => time / Owner.GetTotalAttackSpeed(Projectile.DamageType);
		private float hideTime => time / Owner.GetTotalAttackSpeed(Projectile.DamageType);

		public override string Texture => $"Arathia/Content/Items/Weapons/MoonStoneSword";
        ...
        public void Initialize() {
            if (Owner.HeldItem.ModItem is MoonStoneSword sword) {
                parentSword = sword;
            }
        }
		...
		private void SpawnSubProjectile() {
            ...
            direction.Normalize();
            float targetAngle = MathHelper.ToDegrees((float)Math.Atan2(direction.Y, direction.X));
			ProjectileHelper.ShootProjectileFromProjectile(Projectile, projectileType, 20 * projectileVelocityMultiplier, targetAngle);
		}
		...
		public override void AI() {
			...
			Vector3 colors = parentSword.colors[parentSword.currentStage];
			Lighting.AddLight(Projectile.Center, colors.X, colors.Y, colors.Z);

			if (Main.rand.NextBool(2))
			{
				Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, dustId, Projectile.velocity.X * 0.25f, Projectile.velocity.Y * 0.25f, 150, default(Color), 0.7f);
			}
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
            // Update the sword's stage when hitting an enemy
            if (parentSword != null) {
                if (parentSword.currentStage < MoonStoneSword.MAX_STAGE)
                    parentSword.currentStage++;
                parentSword.hitTimer = MoonStoneSword.STAGE_DECAY_TIME;
            }
		...
		private void ExecuteStrike() {
			float tempExecTime = execTime * parentSword.speedMultipliers[parentSword.currentStage];
			if (CurrentAttack == AttackType.Swing) {
				Progress = MathHelper.SmoothStep(0, SWINGRANGE, (1f - UNWIND) * Timer / tempExecTime);

				if (Timer >= tempExecTime) {
					CurrentStage = AttackStage.Unwind;
				}
			}
			else {
				Progress = MathHelper.SmoothStep(0, SPINRANGE, (1f - UNWIND / 2) * Timer / (tempExecTime * SPINTIME));

				if (Timer == (int)(tempExecTime * SPINTIME * 3 / 4)) {
				...
				if (Timer >= tempExecTime * SPINTIME) {
					CurrentStage = AttackStage.Unwind;
				}
			}
		}
